...
export const getMigrationState = migrationState.getCurrentState;
...
export const getMigrationFilesBeforeVersion = async (version) => {
  // load migrations
  const migrationFiles = await migrations.getMigrationFilenames();
  ...
  const currentState = _.sortBy(await getMigrationState(), 'version');
