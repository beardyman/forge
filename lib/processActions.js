...
function validateAndGetUnrunMigrations(migrations, currentState) {
  ...
  if (!_.isEmpty(unrunMigrations) && !_.isEmpty(currentState) && unrunMigrations[0].version <= _.last(currentState).version) {
...
export const getMigrationsBeforeVersion = async (version) => {
  ...
  log.debug({parsedMigrationFiles});
  ...
  const unrunMigrations = validateAndGetUnrunMigrations(parsedMigrationFiles, currentState);
