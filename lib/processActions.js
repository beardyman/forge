...
import getPlugin from './model/plugin.js';
...
async function getNewMigrations( currentState ) {
  ...
  if ( !_.isEmpty( outOfOrderMigrations )) {
    const error = new Error( 'One or more migrations were added that are older than the current state.  Please resolve.' );
    error.outOfOrderMigrations = outOfOrderMigrations;
    throw error;
  }
...
export function processTasks( migrationFileConfigs, action ) {
  return Promise.each( migrationFileConfigs, async( migrationFile ) => {
    ...
    await actionMethod({ config, plugin: getPlugin(), logger: log });
